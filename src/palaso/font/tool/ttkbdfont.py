...
def main():
    ...
    cmap = cast(table__c_m_a_p, outfont['cmap']).getBestCmap()
    assert cmap is not None, 'Unicode CMAP subtable is required'
    ...
    for k, v in jobs.items():
        ...
        if len(gs) == 1 and gs[0][1][0] == 0 and gs[0][1][1] == 0:
            ...
            cast(table__g_l_y_f, outfont['glyf'])[glyphname] = glyph
            cmap[k] = glyphname
            for g in gs:
                if g[0] == 0:
                ...
                gc.y = g[1][1]
                glyph.components.append(gc)
            cast(table__h_m_t_x, outfont['hmtx']).metrics[glyphname] = [adv, lsb]
            if not firstdone:
                cmap[0xF041] = cmap[k]
                firstdone = True
    table_head = cast(Any, outfont['head'])
    table_os2 = cast(table_O_S_2f_2, outfont["OS/2"])
    try:
        version = float(re.sub(r"[^0-9.]", "", args.version))
        cast(Any, outfont['head']).fontRevision = version
    except ValueError:
        print("Invalid version number")

    t = int(time.time()) + 0x7C259DC0
    table_head.created = t
    table_os2.recalcUnicodeRanges(outfont)
    table_os2.recalcAvgCharWidth(outfont)

    # Names
    from fontTools.ttLib.tables._n_a_m_e import table__n_a_m_e
    table_name = cast(table__n_a_m_e, infont['name'])
    subfamily = table_name.getBestSubFamilyName()
    assert subfamily is not None, 'name table must have a subfamily'
    full = (" " + subfamily) if subfamily.lower() not in ("regular", "standard") else ""
    cright = table_name.getDebugName(0)
    family = table_name.getBestFamilyName()
