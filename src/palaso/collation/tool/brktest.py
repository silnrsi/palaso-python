...
def main():
    ...
    parser.add_option("-c", "--codes", action="store_true",
                      help="final arguments are hex character codes")
    ...
    parser.add_option("-x", "--hex", action="store_true",
                      help="output in hex codes")
    (opts, args) = parser.parse_args()
    if not len(args):
        parser.error("Insufficient arguments, try -h for help")

    if opts.input:
        spec = "".join(open(opts.input).readlines())
    ...
    if opts.codes:
        ...
        text = "".join(open(args[0]).readlines())
    else:
        text = args[0]
